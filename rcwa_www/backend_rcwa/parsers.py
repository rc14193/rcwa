...
def parse_optical_constants(optical_values: str) -> list:
    try:
        ...
        if len(rows) == 1:
            return [float(value) for value in rows[0].split(',')]
        else:
...
def parse_layer(layer: Layer) -> rw.Layer:
    ...
    if layer.hasCrystal:
        ...
        layer.er = complexArray(layer.er)
        layer.ur = np.ones(layer.er.shape)
        rw_layer.homogenous = False
        layer.latticeVectors = [vector[:2] if not layer.is3D else vector for vector in layer.latticeVectors]
        c = rw.Crystal(*layer.latticeVectors, er=layer.er, ur=layer.ur)
        rw_layer.crystal = c
    return rw_layer
